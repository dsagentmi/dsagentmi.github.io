...
interface DataContextType {
    ...
    addOrRemoveSelectedAgent: (agentID: number, attributes: any) => void;
    designspace:any;
...
export const DataContextProvider = ({ children }: { children: ReactNode }) => {
    ...
    const [selectedAgents, setSelectedAgents] = useState<(ISelectedAgent | undefined)>([]);
    ...
    useEffect(() => {
        if (filteredAgents && data) {
            console.log(filteredAgents.map(a => a["ID"]))
            setFilteredPapers(data.references.filter(el => filteredAgents.map(a => String(a["ID"])).includes(el["paperID"])));
        }
    }, [filteredAgents, data]);
    ...
    useEffect(() => {
        if (encryptedUsage) {
            ...
            fetchedData.agent_data = fetchedData.agent_data.map((a, idx) => ({ ...a, agentID: idx }));
